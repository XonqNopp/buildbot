...
class TestMasterShellCommand(TestBuildStepMixin, TestReactorMixin, unittest.TestCase):
    ...
    def test_runtime_timeout_success(self):
        """Test the runtimeTimeout argument."""
        runtime_timeout = 10
        n_ping = 1
        ...
        self.setup_step(master.MasterShellCommand(command=cmd, runtimeTimeout=runtime_timeout))
    ...
    def test_runtime_timeout_failed(self):
        """Test the runtimeTimeout argument aborts the step."""
        runtime_timeout = 1
        n_ping = 10
        cmd = f'ping 127.0.0.1 -n {n_ping}'
